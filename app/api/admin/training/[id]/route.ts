...
import { invalidateTrainingCache } from '@/lib/data'
...
export async function DELETE(
  ...
  try {
    ...
    if (!id) {
      return NextResponse.json({ error: 'Missing training ID parameter' }, { status: 400 })
    }
    ...
    if (isSupabaseConfigured && !user) {
      return NextResponse.json(
        { error: 'Unauthorized: You must be logged in as an administrator to delete training records.' },
        { status: 401 }
      )
    }
    ...
    if (isUuid) {
      const { data, error: selectErr } = await dbClient
        .from('training')
        .select('id, slug, title, image_url, certificate_url')
        .eq('id', id)
        .maybeSingle()

      if (selectErr) {
        console.warn('Training select by UUID warning:', selectErr.message)
      }
      record = data
    }
    ...
    if (!record) {
      ...
      if (selectSlugErr) {
        console.warn('Training select by slug warning:', selectSlugErr.message)
      }
      record = data
    }
    ...
    if (record?.id) {
      const { error: deleteError } = await dbClient.from('training').delete().eq('id', record.id)
      if (deleteError) {
        console.error('Training DB delete error by ID:', deleteError)
        return NextResponse.json(
      ...
      const { error: deleteError } = await dbClient.from('training').delete().eq('id', id)
      if (deleteError) {
        console.error('Training DB delete error directly by ID:', deleteError)
        return NextResponse.json(
          { error: `Database deletion failed: ${deleteError.message}` },
          { status: 500 }
        )
      ...
      const { error: deleteError } = await dbClient.from('training').delete().eq('slug', id)
      if (deleteError) {
        console.error('Training DB delete error by slug:', deleteError)
        return NextResponse.json(
          { error: `Database deletion failed: ${deleteError.message}` },
          { status: 500 }
        )
    ...
    try {
      ...
      if (record?.slug) {
        revalidatePath(`/training/${record.slug}`)
      }
      revalidatePath('/')
    } catch {
    ...
    console.error('Training delete endpoint exception:', err)
